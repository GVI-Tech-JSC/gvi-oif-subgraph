...
import { GGVI_CONTRACT, VESTA_CONTRACT } from "./utils/Constants";
...
export function handleVesta(timestamp: BigInt): void {
  ...
  if (dailyVesta == null) {
    ...
    dailyVesta.depositedGGVI  = BigDecimal.fromString("0")
    dailyVesta.depositedGGVIValue = BigDecimal.fromString("0")
    dailyVesta.save()
  }

  //Update metrics
  let vestaContract = Vesta.bind(Address.fromString(VESTA_CONTRACT))
  dailyVesta.depositedGGVI = toDecimal(vestaContract.getEntireSystemColl(Address.fromString(GGVI_CONTRACT)), 18)
  //TODO USD
  dailyVesta.save()
}
